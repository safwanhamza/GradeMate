...
def demo():
    ...
    with gr.Blocks(theme=gr.themes.Default(primary_hue="red", secondary_hue="pink", neutral_hue = "sky")) as demo:
        vector_db = gr.State()
        qa_chain = gr.State()
        gr.HTML("<center><h1>GradeMate</h1><center>")
        gr.Markdown("""<b>FYP Textbooks RAG Demo</b>
        """)
        with gr.Row():
            with gr.Column(scale = 86):
                gr.Markdown("<b>Step 1 - Upload Course PDF documents and Initialize RAG pipeline</b>")
                with gr.Row():
                    document = gr.Files(height=300, file_count="multiple", file_types=["pdf"], interactive=True, label="Upload PDF documents")
                with gr.Row():
                    db_btn = gr.Button("Process")
                with gr.Row():
                ...
                with gr.Row():
                    qachain_btn = gr.Button("Initialize Question Answering")
                with gr.Row():
